...
@section('title', 'Peserta Event | Admin')
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-sm-6">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <span class="fw-semibold">{{ $registrations->count() }} Peserta</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <div class="stats-card bg-white rounded-4 shadow-lg p-4">
                    ...
                    <div class="stats-grid">
                        <div class="stat-item text-center">
                            <div class="stat-number text-warning fw-bold fs-4">{{ $registrations->where('status', 'registered')->count() }}</div>
                            <div class="stat-label text-muted small">Menunggu</div>
                        </div>
                        <div class="stat-item text-center">
                            <div class="stat-number text-info fw-bold fs-4">{{ $registrations->whereIn('status', ['confirmed', 'paid'])->count() }}</div>
                            <div class="stat-label text-muted small">Dikonfirmasi</div>
                        </div>
                        <div class="stat-item text-center">
                            <div class="stat-number text-success fw-bold fs-4">{{ $registrations->where('status', 'attended')->count() }}</div>
                            <div class="stat-label text-muted small">Hadir</div>
                        </div>
                        <div class="stat-item text-center">
                            <div class="stat-number text-danger fw-bold fs-4">{{ $registrations->where('status', 'cancelled')->count() }}</div>
    ...
    <div class="container">
        ...
        <div class="participants-table-card bg-white rounded-4 shadow-lg overflow-hidden">
            <div class="table-header bg-light p-4 d-flex justify-content-between align-items-center">
                ...
                <div>
                    <a href="{{ route('admin.event-registrations.export-participants', $event->id) }}" class="btn btn-success btn-sm rounded-pill">
                        ...
                        <tr class="participant-row">
                            ...
                            <td class="px-4 py-3">
                                <div class="participant-info">
                                    ...
                                    @if($registration->participant_nim)
                                        <p class="text-muted small mb-0">NIM: {{ $registration->participant_nim }}</p>
                                    @endif
                                </div>
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3">
                                <div class="action-buttons d-flex align-items-center">
                                    ...
                                    <button type="button" class="btn btn-sm {{ $registration->certificate_path ? 'btn-info text-white' : 'btn-outline-secondary' }} rounded-pill" 
                                            data-bs-toggle="modal" 
                                            data-bs-target="#uploadCertificateModal{{ $registration->id }}"
                                            title="{{ $registration->certificate_path ? 'Ganti Sertifikat' : 'Upload Sertifikat' }}">
...
@foreach($registrations as $registration)
<div class="modal fade" id="uploadCertificateModal{{ $registration->id }}" tabindex="-1" aria-labelledby="uploadCertificateModalLabel{{ $registration->id }}" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 shadow-lg rounded-4">
            <div class="modal-header border-0 bg-primary text-white rounded-top-4">
                <h5 class="modal-title fw-bold" id="uploadCertificateModalLabel{{ $registration->id }}">
                    <i class="bi bi-cloud-upload me-2"></i>Upload Sertifikat
                </h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <form action="{{ route('admin.event-registrations.upload-certificate', $registration->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="modal-body p-4">
                    <div class="text-center mb-4">
                        ...
                        <h5 class="fw-bold mb-1">{{ $registration->participant_name }}</h5>
                        <p class="text-muted small mb-0">{{ $registration->registration_number }}</p>
                    </div>
                    ...
                    <div class="mb-3">
                        <label for="certFile{{ $registration->id }}" class="form-label fw-semibold">Pilih File Sertifikat</label>
                        <input type="file" name="certificate_file" class="form-control" accept=".pdf,.jpg,.jpeg,.png" required id="certFile{{ $registration->id }}">
